...
async def receber_dispatch(pedido: DispatchRequest, db: Session = Depends(get_db)):
    try:
        ...
        chave_nfe = pedido.NotaFiscal.Chave if pedido.NotaFiscal and pedido.NotaFiscal.Chave else None
        ...
        uf_remetente = item.Frete.Remetente.Estado or ""
        uf_destinatario = item.Frete.Destinatario.Estado or ""
        ...
        courier_id = None
        if item.Frete.Transportadora and item.Frete.Transportadora.Id:
            try:
